...
BitwiseAndOperation::BitwiseAndOperation(const OperationString &source) : Operation(source)
{

}

void BitwiseAndOperation::performImmediate(vector<AssemblyInstruction>& instructions, const string& target, const string& reg, int val)
{
	...
	if(Utility::isBig(val)) {
		...
		instructions.push_back("and\t" + rm->decodeOperand(target) +
							   ", " + rm->decodeOperand(reg) +
							   ", " + rm->decodeOperand(temp));
		rm->releaseTempRegister(temp);
	} else {
		instructions.push_back("andi\t" + rm->decodeOperand(target) +
							   ", " + rm->decodeOperand(reg) +
							   ", " + Utility::intToHexString(val));
	}
}

vector<AssemblyInstruction> BitwiseAndOperation::getInstructions()
{
	...
	if(rm->isNumber(firstOperand)) {
		...
		if(rm->isNumber(secondOperand)) {
			...
			instructions.push_back("and\t" + rm->decodeOperand(source.target) +
