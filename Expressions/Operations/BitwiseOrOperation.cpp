...
BitwiseOrOperation::BitwiseOrOperation(const OperationString &source) : Operation(source)
{

}

void BitwiseOrOperation::performImmediate(vector<AssemblyInstruction>& instructions, const string& target, const string& reg, int val)
{
	...
	if(Utility::isBig(val)) {
		...
		instructions.push_back("or\t" + rm->decodeOperand(target) +
							   ", " + rm->decodeOperand(reg) +
							   ", " + rm->decodeOperand(temp));
		rm->releaseTempRegister(temp);
	} else {
		instructions.push_back("ori\t" + rm->decodeOperand(target) +
							   ", " + rm->decodeOperand(reg) +
							   ", " + Utility::intToHexString(val));
	}
}

vector<AssemblyInstruction> BitwiseOrOperation::getInstructions()
{
	...
	if(rm->isNumber(firstOperand)) {
		...
		if(rm->isNumber(secondOperand)) {
			...
			instructions.push_back("or\t" + rm->decodeOperand(source.target) +
